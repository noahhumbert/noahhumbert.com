...
# Setup Composer
RUN apt-get update \
    && apt-get install -y unzip git tzdata postfix \
...
# Clear symfony cache and runapache2
CMD ["sh", "-c", "php bin/console cache:clear && php bin/console cache:warmup && apache2-foreground"]
# Switch to www-data
USER www-data
